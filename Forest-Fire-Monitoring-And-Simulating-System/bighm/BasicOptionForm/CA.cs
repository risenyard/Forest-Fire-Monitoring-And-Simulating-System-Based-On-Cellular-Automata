...
{   
    public partial class CA : Form
    {
        ...
        //初始化
        public CA(AxMapControl mapcontrol)
        {
            InitializeComponent();
            cb_box = new List<ComboBox>() { cb_FireMonitoringResult, cb_Temperture, cb_WindSpeedRank, cb_Humidity,cb_WindSpeed,cb_DEM };
            //cb_box.Add(cb_DEM);cb_box.Add(cb_FireMonitoringResult);
            //cb_box.Add(cb_Humidity);cb_box.Add(cb_LULC);cb_box.Add(cb_NDVI);
            //cb_box.Add(cb_Temperture);cb_box.Add(cb_WindSpeed);
        ...
        private void iconButton1_Click(object sender, EventArgs e)
        {
            ...
            //元胞运算
            CAMethods calculateCA = new CAMethods();
            double[,] CAresult = calculateCA.FireCA_double(background, doubleteam[0], doubleteam[1], doubleteam[2], doubleteam[3], doubleteam[4], doubleteam[5], Convert.ToDouble(numericUpDown1.Value), Convert.ToDouble(numericUpDown2.Value), (int)numericUpDown3.Value);

            //输出图层
            if (CAresult != null)
            {
                outLayers = outToLayer(Layers[Layers.Count - 1], CAresult);
                //两个成图序列
                timeseries = calculateCA.timeseries;
                fireseries = calculateCA.fireseries;
